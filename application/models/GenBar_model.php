...
class Genbar_Model extends CI_model
{
	...
	public function getshow_query($id_karyawan)
	{
		...
		$this->db->select('a.id_karyawan,a.nama_karyawan,b.nama_jabatan,d.nama_divisi,c.nama_gedung');
		$this->db->from('karyawan as a,jabatan as b,gedung as c,divisi as d');
		$this->db->where('b.id_jabatan = a.jabatan');
		$this->db->where('a.gedung_id = c.gedung_id');
		$this->db->where('d.id_divisi = a.divisi');
